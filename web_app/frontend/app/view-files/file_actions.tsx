...
export interface file_actions_props {
  ...
  setSpiderLoading: React.Dispatch<React.SetStateAction<boolean>>; // deprecated
  spiderLoading: boolean; // deprecated
}
...
const FileActions: React.FC<file_actions_props> = ({
  ...
  files,
  setSelected,
  setSpiderLoading: _setSpiderLoading, // keep props to avoid breaking callers
  spiderLoading: _spiderLoading,       // but not used
}) => {
  return (
    <div className="flex gap-2 mt-4 mb-8">
      <label
        className={`px-4 py-2 rounded font-medium text-center flex items-center justify-center ${
          addLoading
      ...
      >
        ...
        <span>Add zip files or sqlite files</span>
        <input
          type="file"
          multiple
          hidden
          accept=".zip,application/zip,.sqlite,.db"
          ...
          title="Upload zip or sqlite database files"
        />
      </label>
